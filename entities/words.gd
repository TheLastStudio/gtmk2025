...
const LETTER_BLACKLIST := "[^a-zA-Zа-яА-ЯёЁїЇіІєЄґҐ]*"
...
func build_pattern(word: String) -> String:
	...
	for c in normalized_word:
		...
		pattern += "[" + "".join(escaped) + "]" + LETTER_BLACKLIST
	return pattern
